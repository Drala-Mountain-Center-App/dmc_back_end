...
RSpec.describe 'create video with GraphQL' do
  ...
  describe 'sad path' do
    it 'cannot create a video without a vimeo_id', :vcr do
      query = <<~GQL
        mutation{
          createVideo(input: {
            vimeoId: nil,
      ...
      expect(error["message"]).to eq("Argument 'vimeoId' on InputObject 'CreateVideoInput' has an invalid value (nil). Expected type 'Int!'.")
    end
  end
end